...
public class StashFromHandAction extends AbstractGameAction {
    public static final String[] TEXT;
    private final boolean anyNumber;
    private final boolean endOfTurn;
    private ArrayList<AbstractCard> invalidTargets = new ArrayList<>();

    public StashFromHandAction(AbstractCreature source, int amount, boolean anyNumber) {
        this(source, amount, anyNumber, false);
    }

    public StashFromHandAction(AbstractCreature source, int amount, boolean anyNumber, boolean endOfTurn) {
    ...
    public void update() {
        if (this.duration == 0.5F) {
            ...
            AbstractDungeon.handCardSelectScreen.open(TEXT[9], this.amount, false, anyNumber, false, false, anyNumber);
        } else {
    ...
    static {
        TEXT = CardCrawlGame.languagePack.getUIString("bronze:MiscStrings").TEXT;
    }
}
