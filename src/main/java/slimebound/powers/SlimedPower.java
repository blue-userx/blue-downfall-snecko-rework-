...
public class SlimedPower extends AbstractPower {
    ...
    public boolean dontRemoveOnce = false;
    ...
    public void onInitialApplication() {
        addToBot(new VFXAction(new FakeFlashAtkImgEffect(this.owner.hb.cX, this.owner.hb.cY, new Color(Color.PURPLE), 1F, false, 0.6F)));
        addToBot(new VFXAction(new SlimeDripsEffectPurple(this.owner.hb.cX, this.owner.hb.cY, 4), 0.05F));
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if (info.type == DamageInfo.DamageType.NORMAL && info.owner == AbstractDungeon.player) {
            ...
            if (this.source.hasPower(GoopArmorPower.POWER_ID)) {
                addToBot(new GainBlockAction(this.source, this.source, this.source.getPower(GoopArmorPower.POWER_ID).amount));
                this.source.getPower(GoopArmorPower.POWER_ID).flash();
            }

            if (this.source.hasPower(GoopIntoPoisonPower.POWER_ID)) {
                addToBot(new ApplyPowerAction(this.owner, this.source, new PoisonPower(this.owner, this.source, this.source.getPower(GoopIntoPoisonPower.POWER_ID).amount), this.source.getPower(GoopIntoPoisonPower.POWER_ID).amount, true, AbstractGameAction.AttackEffect.POISON));
            ...
            if (dontRemoveOnce) {
                dontRemoveOnce = false;
            } else if (this.owner.hasPower(PreventSlimeDecayPower.POWER_ID)) {
                addToBot(new ReducePowerAction(this.owner, this.owner, this.owner.getPower(PreventSlimeDecayPower.POWER_ID), 1));
            } else {
