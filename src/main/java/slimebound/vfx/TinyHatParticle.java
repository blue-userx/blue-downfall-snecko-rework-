...
public class TinyHatParticle extends com.megacrit.cardcrawl.vfx.AbstractGameEffect {
    ...
    public TinyHatParticle(AbstractPlayer p) {
        ...
        if(CharacterSelectScreenPatches.characters[1].reskinCount <= 0){
            this.img = TextureLoader.getTexture("slimeboundResources/SlimeboundImages/relics/tinybowlerhatinverted.png");
            W = img.getWidth();
            H = img.getHeight();
            xOffset = 20;
            yOffset = 0;
            ...
            this.img = TextureLoader.getTexture("reskinContent/img/Slimebound/Slaifu/animation/hat.png");
            W = img.getWidth();
            H = img.getHeight();
            xOffset = -17;
            yOffset = -2;
...
//        if(AbstractDungeon.player.hasPower("Slimebound:DuplicatedFormPower" ) && reskinContent.slimeReskinAnimation ){duplicatedScale = 1.5f;}else {duplicatedScale = 1.0f;}
