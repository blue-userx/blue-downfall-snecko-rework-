    ...
    public void update() {
        ...
        for (int i = 0; i < this.amount; i++) {
            ...
            if (!possCards.isEmpty()) {
                AbstractCard tar = (AbstractCard) Wiz.getRandomItem(possCards, AbstractDungeon.cardRandomRng);
                tar.upgrade();
        ...
        if (upgradedAnyCard) {
            if (DEST_X == -1.0F && DEST_Y == -1.0F) {
                Hitbox hb = (Hitbox)ReflectionHacks.getPrivate(AbstractDungeon.overlayMenu.combatDeckPanel, DrawPilePanel.class, "hb");
                DEST_X = hb.cX;
                DEST_Y = hb.cY;
            }
